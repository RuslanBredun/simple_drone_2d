...
A compact Python solution for the 2D drone test task.

## Project structure
- `drone_simple.py` is the completed minimal solution for the assignment.
- `drone.py` is the extended experimental sandbox where different controllers, wind, moving targets, and UI controls can be tested.
- `controller.py` contains the controller implementations shared by both runtimes.
...
What it is:
- Minimal implementation of the assignment.
- 2D drone with gravity and thrust only.
- Absolute pitch command and throttle command.
- Random target regeneration after each successful hit.
- Compact `hold` and `fast` autopilot modes.

Controls:
- `1`: `hold`
- `2`: `fast`
- `R`: reset drone and generate a new target
- `Space`: pause
- `Esc`: exit
...
What it is:
- Extended sandbox for experiments and controller tuning.
- Includes UI sliders, multiple auto modes, moving target, wind, intercept visualization, and trajectory debugging.
...
3. Available controls:
   ...
   - Checkbox `Moving target`: enable or disable target motion; enabled by default
...
- `Moving target` is enabled by default. While enabled, the target moves continuously inside the upper part of the world with a smoothly changing random velocity, and its current velocity is shown by an arrow near the target marker.
...
- `auto_hold` uses a PID-style controller that tries to reach and hold the target.
...
- `controller.py` defines the autopilot/controller data contracts plus the hold, fast-hit, and intercept controllers.
- `drone_body.py` defines the shared drone rigid-body representation used by both runtimes.
- `target_utils.py` contains shared target generation helpers.
